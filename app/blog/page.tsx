  ...
  if (!posts || posts.length === 0) {
    return (
      <div className="max-w-6xl mx-auto py-20 px-4 text-center">
        ...
        <a href="/admin/posts/new" className="px-4 py-2 bg-sky-500 text-white rounded-lg">
          Create First Post
        </a>
      </div>
    );
        ...
        {posts.map((post) => (
          <div key={post.id} className="relative group">
            <Link href={`/blog/${post.slug}`} className="block">
              ...
              <h2 className="text-xl font-semibold group-hover:text-sky-600 transition">
                {post.title}
              </h2>
            ...
            {/* Edit Button - Only shows in development mode */}
            {process.env.NODE_ENV === 'development' && (
              <Link href={`/admin/posts/edit/${post.id}`}>
                <button className="absolute top-2 right-2 p-1.5 bg-white rounded-lg shadow-md text-xs hover:bg-gray-100 transition z-10">
                  ✏️ Edit
                </button>
              </Link>
            )}
